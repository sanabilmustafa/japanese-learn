  ...
  return (
    <div>
        <nav className="bg-white border-gray-200">
            <div className="max-w-screen-xl flex items-center mx-auto p-5">
                <a href="#" className="flex items-center">
                    <span className=" text-2xl font-semibold whitespace-nowrap text-black">JAPANESE</span>
                </a>
                <button data-collapse-toggle="navbar-dropdown" type="button" onClick={() => setNavExpand(() => !navExpand)} className={` inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-${color}-200 `} aria-controls="navbar-dropdown" aria-expanded="false">
                    <span className="sr-only">Open main menu</span>
                    <svg className="w-5 h-5" aria-hidden="true" xmlns="http: www.w3.org/2000/svg" fill="none" viewBox="0 0 17 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 1h15M1 7h15M1 13h15"/>
                    </svg>
                </button>
                <div className={`${navExpand ? 'block' : 'hidden'} w-full md:block md:w-auto" `} id="navbar-dropdown">
                    <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white ">
                        <li>
                        <a href="/" className={`block py-2 pl-3 pr-4 text-${color}-400 bg-${color}-700 rounded md:bg-transparent md:text-${color}-700 md:p-0`} aria-current="page">Home</a>
                        </li>
                        <li>
                          ...
                          <li>
                          <a href="#" className={`block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-${color}-400 md:p-0`}>Contact</a>
                          </li>
